...
function Customers() {
  ...
  const res = await axios.get(`${serverUrl.replace(/\/$/, '')}/clients/users`, {
  ...
  `${serverUrl.replace(/\/$/, '')}/clients/users/${editUser._id}`,
        editUser,
        {
          withCredentials: true,
          headers: { Authorization: `Bearer ${token}` },
      ...
      const updatedUser = res.data.data || res.data || editUser;

    const userWithNewRefs = {
      ...updatedUser,
      address: updatedUser.address ? JSON.parse(JSON.stringify(updatedUser.address)) : {},
    };
      ...
      setUsers((prev) =>
        prev.map((u) => (u._id === userWithNewRefs._id ? userWithNewRefs : u))
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
  await axios.delete(`${serverUrl.replace(/\/$/, '')}/clients/users/${id}`, {
        withCredentials: true,
        headers: { Authorization: `Bearer ${token}` },
      });
